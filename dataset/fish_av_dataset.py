...
from utils.video_transform import CenterCropVideo, RandomHorizontalFlipVideo
...
class Fish_Video_Dataset(Dataset):
    def __init__(self, seed, epoch, sample_rate, split):
        ...
        train_v_dict, test_v_dict, val_v_dict = data_generator(
            self.seed, test_sample_per_class=100
        )
        if self.split == 'train':
    ...
    def __getitem__(self, index):
        ...
        pickle_path = os.path.join(
            save_path, str(self.epoch), self.split, '%s.pkl' % index
        )
...
def get_dataloader(
    ...
    dataset = Fish_Video_Dataset(
        split=split, seed=seed, sample_rate=sample_rate, epoch=epoch
    )
...
if __name__ == '__main__':
    ...
    for i in range(17, 100):
        ...
        # test_loader = get_dataloader(split='test', batch_size=20, seed=25, num_workers=8, sample_rate=128000, epoch=i)
        #
        # print(" \n Start save test_loader")
        # for item in tqdm(test_loader):
        #     pass
        #
        # print(" \n Start save val_loader")
        # val_loader = get_dataloader(split='val', batch_size=20, seed=25, num_workers=8, sample_rate=128000, epoch=i)
        # for item1 in tqdm(val_loader):
        #     pass
