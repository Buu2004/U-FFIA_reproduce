    ...
    for epoch in range(max_epoch):
        ...
        for data_dict in tqdm(train_loader):
            ...
            if modality_drop_prob < modality_dropout:
                if audio_drop_prob < audio_dropout:
                    ...
                    video_input = data_dict['video_form_all'].permute(0, 2, 1, 3, 4).to(device)
                    Pre_video_output = Pre_VideoModel(video_input)
                    KL_video_loss = soft_loss(
                        F.log_softmax(video_output['clipwise_output'], dim=1),
                        F.softmax(Pre_video_output['clipwise_output'][0] / temp, dim=1),
                    )
                    target_dict = {'target': data_dict['target']}
                    loss = (
                        0.5 * loss_func(video_output, target_dict) + 0.5 * KL_video_loss
                    )
                else:
                    # logger.info("Now is using only audio modality")
                    audio_output = model(data_dict['waveform'], None, 'a')
                    Pre_audio_output = Pre_AudioModel(data_dict['waveform'])
                    ...
                    target_dict = {'target': data_dict['target']}
                    loss = (
                        0.5 * loss_func(audio_output, target_dict) + 0.5 * KL_audio_loss
                    )
            else:
        ...
        if epoch % 1 == 0:
            ...
            if val_acc_video > best_acc_video:
                ...
                save_model(
                    os.path.join(ckpt_dir, 'video_best.pt'),
                    model,
                    optimizer,
                    best_acc_video,
                    best_epoch_video,
            ...
            if val_acc_av > best_acc_av:
                ...
                save_model(
                    os.path.join(ckpt_dir, 'av_best.pt'),
                    model,
                    optimizer,
                    best_acc_av,
                    best_epoch_av,
            ...
            model.train()
        logger.info(
            f'val_best_acc_audio: {best_acc_audio}, best mAP_audio:{best_mAP_audio}, best_epoch: {best_epoch_audio}'
        )
        logger.info(
            f'val_best_acc_video: {best_acc_video}, best mAP_video:{best_mAP_video}, best_epoch: {best_epoch_video}'
        )
        logger.info(
            f'val_best_acc_av: {best_acc_av}, best mAP_av:{best_mAP_av}, best_epoch: {best_epoch_av}'
        )
