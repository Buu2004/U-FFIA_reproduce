...
class Audio_Frontend(nn.Module):
    ...
    def forward(self, input):
        ...
        x = self.spectrogram_extractor(
            input
        )  # (batch_size, 1, time_steps, freq_bins) 100,1,251,1025
        x = self.logmel_extractor(
            x
        )  # (batch_size, 1, time_steps, mel_bins) 100,1 251,64
...
class AudioModel_Trasformer(nn.Module):
    def __init__(self, frontend, backbone, **kwargs):
        ...
        old_pretrained_encoder = torch.load(
            '/mnt/fast/nobackup/users/mc02229/FishMM/pretrained_models/xxsmodel_best.pth.tar'
        )['state_dict']
        ...
        pretrained_encoder = {
            k.replace('module.', ''): v for k, v in old_pretrained_encoder.items()
        }
        trained_list = [
            i
            for i in pretrained_encoder.keys()
            if not ('fc' in i or i.startswith('stem.0'))
        ]
...
class AudioModel_ownpretrained(nn.Module):
    def __init__(self, frontend, backbone, **kwargs):
        ...
        old_pretrained_audio_encoder = torch.load(
            '/mnt/fast/nobackup/users/mc02229/FishMM/pretrained_models/audio-visual_pretrainedmodel/audio_best.pt'
        )['model_state_dict']
        dict_new = self.audio_encoder.state_dict().copy()
        dict_new_frontend = self.audio_frontend.state_dict().copy()
        pretrained_audio_encoder = {
            k.replace('backbone.', ''): v
            for k, v in old_pretrained_audio_encoder.items()
        }
        pretrained_audio1_encoder = {
            k.replace('frontend.', ''): v for k, v in pretrained_audio_encoder.items()
        }
        pretrained_audio_encoder = {
            k: v for k, v in pretrained_audio1_encoder.items() if k in dict_new
        }
        pretrained_frontend = {
            k: v for k, v in pretrained_audio1_encoder.items() if k in dict_new_frontend
        }
        trained_list = [
            i
            for i in pretrained_audio_encoder.keys()
            if not ('head' in i or 'pos' in i)
        ]
        for i in range(len(trained_list)):
        ...
        trained_list1 = [
            i for i in pretrained_frontend.keys() if not ('head' in i or 'pos' in i)
        ]
        ...
        self.slf_attn = MultiHeadAttention(
            n_head=1, d_model=1024, d_k=1024, d_v=1024, dropout=0.1
        )
...
class AudioModel_Cnn6(nn.Module):
    def __init__(self, frontend_pre, backbone_pre, backbone, **kwargs):
        ...
        self.backbone = backbone
        pretrained_audio_encoder = torch.load(
            '/mnt/fast/nobackup/users/mc02229/FishMM/pretrained_models/PANNs/Cnn6.pth'
        )['model']
        dict_new = self.audio_encoder.state_dict().copy()
        # pretrained_audio_encoder = {k:v for k, v in pretrained_audio_encoder.items() if k in dict_new}
        trained_list = [
            ...
            if not (
                'fc_audioset' in i
                or i.startswith('bn0')
                or i.startswith('spec')
                or i.startswith('logmel')
            )
        ]
        for i in range(len(trained_list)):
            dict_new[trained_list[i]] = pretrained_audio_encoder[trained_list[i]]
...
class AudioModel_pre_Cnn10(nn.Module):
    def __init__(self, frontend, backbone, **kwargs):
        ...
        self.audio_encoder = backbone
        self.audio_frontend = frontend
        pretrained_audio_encoder = torch.load('pretrained_models/Cnn10.pth')['model']
        dict_new = self.audio_encoder.state_dict().copy()
        trained_list = [
            i
            for i in pretrained_audio_encoder.keys()
...
class AudioModel_MV2(nn.Module):
    def __init__(self, frontend_pre, backbone_pre, **kwargs):
        ...
        self.audio_encoder = backbone_pre
        self.audio_frontend = frontend_pre
        pretrained_audio_encoder = torch.load('pretrained_models/audio_best.pt')[
            'model_state_dict'
        ]
        dict_new = self.audio_encoder.state_dict().copy()
        pretrained_encoder = {
            k.replace('backbone.', ''): v for k, v in pretrained_audio_encoder.items()
        }
        pretrained_encoder = {
            k.replace('frontend.', ''): v for k, v in pretrained_encoder.items()
        }
        # pretrained_audio_encoder = {k:v for k, v in pretrained_audio_encoder.items() if k in dict_new}
        trained_list = [
            i
